...
Write-ScriptLog "--- Starting Liongard Agent Installation (v2.3) ---"
...
# --- 5. Pre-Flight Checks ---
Write-ScriptLog "Checking for existing Liongard Agent Service..."
if (Get-Service | Where-Object { $_.Name -like "*Liongard*" -or $_.DisplayName -like "*Liongard*" }) {
    Write-ScriptLog "A Liongard Agent service is already present. Aborting." "WARN"
    Exit-Smart 0
}
...
try {
    ...
    Write-ScriptLog "Verifying connectivity..."
    $Request = [System.Net.WebRequest]::Create($DownloadUri); $Request.Method = "HEAD"; $Response = $Request.GetResponse(); $Response.Close()
}
catch { Write-ScriptLog "Pre-check failed: $_" "ERROR"; Exit-Smart 1 }

# --- 6. Download & Install ---
Write-ScriptLog "Downloading MSI..."
try {
...
Write-ScriptLog "Installing Liongard Agent..."
