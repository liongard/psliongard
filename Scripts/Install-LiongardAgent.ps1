...
<#
    ...
    This variant keeps the richer validation, environment resolution, MachineGuid
    collision handling, optional pre-uninstall, optional remote agent deletion,
    transcript logging, and install verification behavior from the March script
    while using the EXE wrapper instead of the MSI directly.
#>
...
# MSI argument: AUTOUPDATE. Use the public MSI property name so the EXE bundle can
# forward it without tripping the bundle's non-overridable variable handling.
$AutoUpdate                 = $null
...
function Invoke-LiongardAgentUninstall {
    ...
    $entries = @(Get-UninstallEntries -Names $searchNames -ProductCodes $productCodes | Where-Object { $_ })
...
function Write-ConfigurationSnapshot {
    ...
    Write-SectionValue -Label 'ApiTokenKey (masked)' -Value (Mask-SecretValue -Value $ApiTokenKey)
    Write-SectionValue -Label 'ApiTokenSecret (masked)' -Value (Mask-SecretValue -Value $ApiTokenSecret)
...
function Get-LiongardAgentsFiltered {
    ...
    try {
        ...
        $detail = Get-HttpErrorDetail -Exception $_.Exception
        if ($detail) {
            Write-Warning "Filtered agent lookup failed: $($_.Exception.Message) | Response: $detail"
        }
        else {
...
function Remove-LiongardAgentRecord {
    ...
    try {
        ...
        $detail = Get-HttpErrorDetail -Exception $_.Exception
        if ($detail) {
            Write-Warning "Liongard API agent delete failed: $($_.Exception.Message) | Response: $detail"
        }
        else {
...
function Test-VisualCppInstalled {
    if ((Get-VisualCppEntries).Count -gt 0) {
        return $true
    }
...
function Resolve-ComponentValidationStatus {
    ...
    switch ($Name) {
        'AgentMsi' {
            ...
            $localDetails = Get-LiongardAgentDetectionDetails
        ...
        'VisualCpp' {
            ...
            $localDetail = if ($installed) {
                $vcEntries = Get-VisualCppEntries | Select-Object -First 4 -ExpandProperty Name
        ...
        'Npcap' {
            ...
            $npcapDetails = Get-NpcapDetectionDetails
            $localDetail = if ($installed) {
...
try {
    ...
    try {
        $componentValidation = Get-ComponentValidationResults -LogPath $logPath -InstallNetworkIQRequested $InstallNetworkIQ
